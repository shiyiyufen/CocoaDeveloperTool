...
{
	self.resultV.string = @"";
	if (self.textf.stringValue.length && self.textV.string.length)
	{
		[[MyTool shared] saveLastContent:self.textf.stringValue content:self.textV.string];
...
- (void)windowDidLoad
{
	...
	if ([defaults stringForKey:REGULAR])
	{
		self.textf.stringValue = [defaults stringForKey:REGULAR];
	}
	if ([defaults stringForKey:CONTENT])
	{
		self.textV.string = [defaults stringForKey:CONTENT];
	}
